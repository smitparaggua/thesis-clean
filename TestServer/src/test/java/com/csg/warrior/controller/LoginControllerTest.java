...
public class LoginControllerTest {
    ...
    private UserMobileKeyService userMobileKeyService = mock(UserMobileKeyService.class);

    @Before
    public void setup() {
        loginController.setUserService(userService);
        loginController.setUserMobileKeyService(userMobileKeyService);
    }

    @Test
    public void getKeyStatusOfNotWarriorUsers() {
        WarriorKeyStatus expected = new WarriorKeyStatus(false, false);
        when(userMobileKeyService.reportMobileKeyStatusOf(any(User.class), anyBoolean()))
                .thenReturn(expected);
